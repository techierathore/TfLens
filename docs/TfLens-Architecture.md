...
**Status:** Target (greenfield) — **amended 2026-09-08: the 2026-09-07 stream fields — `sort` and `what` on a miss and a fourth `review` record kind (ADR-029), a requirement keyed by `(project, req_id)` and never by id alone (ADR-030), `duration_s` and `attempt` derived at read time and never backfilled (ADR-031), and an unrecognised value in an open vocabulary rendered rather than filtered (ADR-032)**; **amended 2026-09-01 (round 2, post-build): three clauses corrected against the shipped `tf-metrics.sh` reference script after the F-EFFORT parity run (ADR-028)**; **amended 2026-09-01: phase effort and efficiency from both frameworks — the Playbook's transient phase output arrives through the existing import mode rather than a second ingest path (ADR-023), Playbook misses reuse the three existing miss tables with their axes as distinct nullable columns (ADR-024), three normalized Playbook phase tables (ADR-025), the fan-out denominator is a scope predicate rather than a coalesce (ADR-026), and wall-clock / observed-active / human effort are three types rather than three names for one number (ADR-027)**; prior amendments — 2026-08-26: AppManager identity (no local users), per-user public-repo management, GitHub SSO deferred to Phase 2, dark-first collapsible shell; round 2 same day: PostgreSQL replaces SQLite (ADR-015), Framework as a provenance axis with the full Playbook report set in Phase 3 (ADR-016), harness columns claude-code/opencode/codex (ADR-017); **amended 2026-08-28: miss telemetry and rework economics — a fifth stream carrying three record kinds on one file (ADR-018), apportioned cost as a distinct result type (ADR-019), amendments folded at read time and never at ingest (ADR-020); round 2 same day: imported telemetry for private/corporate repos, folded into the Repos screen as a second mode — origin displayed but never pooled on (ADR-021), the uploaded bundle's sha256 standing where a commit SHA does (ADR-022)**
...
| UI | Blazor Server (Interactive Server render mode) + TrBlazeUI | TrBlazeUI.Components / Icons.Lucide / Primitives 2.0.7 (private GitHub Packages feed; as built 2026-09-15) | Single web head `src/TfLens`. Dogfoods TrBlazeUI where it fits (shell, nav, cards, grid, badges, alerts, dialogs). |
...
**`MissMetrics` extended (2026-09-08).** The four steps are unchanged; what changes is what each of them covers.

- **Fold.** The allowlist grows from one field to three — `WhyMissed`, **`Sort`** (closed vocabulary `spec` | `unsaid` | `weak-check` | `ignored`) and **`What`** (free prose, so validated only as non-empty; there is no vocabulary to close). Most amendments in the estate today complete `Sort` on records written before the field existed, which is precisely the case the read-time fold was built for. The fold already counts what it applies; that count is now **published** rather than merely held (BRD-176), because a reader who cannot tell a value that was answered from one answered later is reading a distribution whose provenance is invisible.
- **Floor.** `FIELD_SINCE` gains `sort` and `what`, both `2026-09-07`, in the same table and the same code path as `why_missed` and `LATE_GATES`. This is the whole of BRD-172's implementation: declaring the floor once, in data, makes `n of N sorted` and the separate *predates the field* count automatic for every figure built on either field, rather than a rule each new chart has to remember. It matters immediately and will for months — **286 of 359 misses across the estate carry no `sort`**, and on TfLens's own repository the figure is every record it has.
- **Shape.** `review` records are read into their own `MissReview` collection and returned as a `ReviewCost(int Corrections, Figure ProducedCost, Figure CorrectionCost)` whose two cost members are **copied from the record and never computed** — the emitter copied them from the two runs it names, and TfLens has no arithmetic path that could produce them. An absent figure is `Figure.NotApplicable`, which renders *not available*; there is no code path that yields `0`. `MissReview` rows never enter any miss count, distribution or filter.

**Two derivations, in the engine and not in the store (2026-09-08).** `RunDuration` and `GateAttempt` are read-time derivations, computed every time a figure is built and written nowhere (ADR-031). `RunDuration` returns `duration_s` where the record carries one and otherwise `ended − started`, treating `ts` as `ended` where `ended` is absent too, and it returns the **derived count beside the value** so no time total can be rendered without it. `GateAttempt` counts prior live records for the same `(project, req_id)` in stream order and never alters a record that already carries an `attempt`. Both are `AnalysisResult`-level and therefore inherited by the pages, the export and parity at once; `rebuild` re-derives identical values from the raw archive, exactly as the amendment fold does.

**`(project, req_id)` is the requirement key (2026-09-08, ADR-030).** `ReqKey(string Project, string ReqId)` replaces the bare string in `TaintSet`, `FirstPassRate`, `GateDistribution`, `EscapeRate`, `GateAttempt`, every export key and every parity comparison. Every project emits a `REQ-UI-001`; the bare key fuses them and reports one requirement where there are many, which moved the framework's own combined first-pass rate from **72% to 48%** when it was fixed there. TfLens computes it the pre-fix way today.
...
erDiagram
  ...
  SyncState ||--o{ MissReview : "UserId, Repo"
...
*Added 2026-09-08 — the fourth miss table (F-MISS, BRD-115).* `review` records are a sibling of the other three, not a column on `Miss`: a review is about a **phase's output**, not about any one miss, and it carries no `MissId` to hang from. `Miss` gains the two new fields in the same pass.

```sql
-- the two fields added to a miss record on 2026-09-07 (BRD-170, BRD-173)
ALTER TABLE "Miss" ADD COLUMN IF NOT EXISTS "Sort" text NULL;   -- spec | unsaid | weak-check | ignored; NULL = predates the field
ALTER TABLE "Miss" ADD COLUMN IF NOT EXISTS "What" text NULL;   -- one sentence in the owner's words; the ONLY prose that travels
...
`ITelemetryStore` gains `ReadMissesAsync`, `ReadMissFixesAsync`, `ReadMissAmendsAsync` and — *2026-09-08* — `ReadMissReviewsAsync`, all mirroring `ReadGatesAsync`'s signature; `UpsertAsync` handles the **four** miss `ParseResult` collections; and **`DeleteRepoDataAsync` must purge all four** — missing one leaves orphaned rows that reappear in every figure, which is the worst class of bug in a product whose promise is correct numbers. `SyncState` gains a `misses` row per repo, so Coverage's per-repo stream table goes from four rows to five. `AuthSession` token columns are encrypted at rest with ASP.NET Data Protection. `PbEvent` columns are provisional until the real file is parsed (Phase 3 schema-discovery). Nullable-vs-absent is preserved: an absent optional field is stored as `NULL`, never `0` (SCHEMA.md §2.5).
...
| *(2026-09-08)* every `req_id` key in the table above | `ReqKey(Project, ReqId)` | **never the bare id** — every project emits a `REQ-UI-001`; the bare key fused them and read 72% where the truth was 48% (ADR-030) |
| *(2026-09-08)* `duration_s` absent on a run | `RunDuration` | `ended − started`, and `ts` for `ended` where that is absent too; **derived at read time, never written**; the derived count travels with the value (ADR-031) |
| *(2026-09-08)* `attempt` absent on a gate | `GateAttempt` | 1 + prior live records for the same `ReqKey` in stream order; a record carrying one is never altered; derived count published beside the rate (ADR-031) |
| *(2026-09-08)* misses block: `sort{}` / `sort_n` / `sort_predates_field` | `MissMetrics.Sort` | denominator is records **eligible** to carry it (`FIELD_SINCE = 2026-09-07`); *predates the field* is its own count and never pools into an *unsorted* bucket |
| *(2026-09-08)* misses block: amendments folded | `MissMetrics.AmendedN` | already computed by the read-time fold; now published beside any distribution it touched (BRD-176) |
| *(2026-09-08)* gates carrying `req_class: "FR"` | `Segment.ByReqClass` | the framework grading **itself**; its own segment or excluded — never averaged with `UI` / `FN` / `RAG` / `NFR`, the same wall as `project_type` |
| *(2026-09-08)* an unrecognised `cmd` / `verdict` / `harness` | `OpenVocabulary` | rendered as it stands and counted; **no whitelist filter anywhere** (ADR-032) |
| `FIELD_SINCE` | *(extended 2026-09-08)* | `+ sort`, `what` — both `2026-09-07`, same table, same code path |

Four guarantees are enforced by type, not by discipline: (1) `AnalysisResult` has no member that could hold a cross-`project_type` or cross-provenance rate; (2) `Figure` is a discriminated union of `Value` / `InsufficientData(n)` / `NotApplicable` — a page binding a `Figure` cannot print a number for an `InsufficientData` case; (3) *(2026-08-28)* `MissCost` exposes `Sole`, `Apportioned` and `NoneCount` and nothing else, so a page cannot render a blended measured-plus-apportioned cost — there is no property to bind; and (4) *(2026-09-01)* every phase-effort figure is returned **wrapped in the count it rests on** — a token total arrives as `TokenWindow(Figure Tokens, int MeasuredN, int UnmeasuredN)` and a fan-out figure as `FanoutObservation(double? Spawns, int ObservedN, int UnobservedNotTree, int UnobservedPredatesField)` — so a page cannot render a spawn count without also holding the number of runs that could have been observed. *(Corrected 2026-09-01: `Spawns` is `double?`, not `Figure`. The wrapper is what makes the denominator inescapable; the `MIN_N` refusal belongs to `TokenWindow.Tokens`, which the script does floor, and not here, which it does not.)* *(5) — 2026-09-08* — the same technique reaches the requirement key and the two derivations: `ReqKey` is a record type, not a string, so a rollup **cannot** be keyed on the id alone by forgetting to include the project (ADR-030); and `RunDuration` and `GateAttempt` each return their value **wrapped in the count that was derived** — `DerivedFigure(T Value, int DerivedN)` — so a page or an export cannot render a time total or a first-pass rate without also holding how much of it TfLens worked out rather than read (ADR-031). `ReviewCost(int Corrections, Figure ProducedCost, Figure CorrectionCost)` is the fourth application of the `MissCost` shape and the reason a review cost cannot silently become a zero: its members are `Figure`, and `Figure.NotApplicable` has no numeric value to print.

The technique is the same each time: **make the wrong number unrepresentable rather than forbidden** (ADR-007, ADR-019, ADR-026, ADR-030, ADR-031). A unit test asserts the engine output on the fixture set equals a checked-in `reference.json` produced by the script.
...
- **ADR-029 — `review` is a fourth record kind with its own table, not a column on `Miss` (2026-09-08).** Reason: a review record is about a **phase's output** and carries no `miss_id` to hang from — modelling it as a nullable column set on `Miss` would put a row in the miss table that is not a miss, and every existing count, distribution and filter over that table would have to learn to exclude it. A sibling table excludes it structurally. The urgency is that the *unamended* code is actively destructive rather than merely incomplete: `AddRecord`'s three-arm dispatch sends every `review` record to `InvalidLines++`, so a product whose subject is mistakes silently discards the only records that price a **specification** defect. The cost columns are nullable with no default for the same reason `MissCost` exists — a `DEFAULT 0` would render documents free, which is the specific wrong number this record was added upstream to prevent.
- **ADR-030 — A requirement is keyed by `(project, req_id)`, expressed as a record type and never as a bare string (2026-09-08).** Reason: every project emits a `REQ-UI-001`, so a bare-string key fuses one requirement per project into one requirement overall, and the resulting figure looks entirely normal — it is the §1 failure mode exactly, a *plausible wrong number* that would be exported and quoted. On the framework's own data the combined first-pass rate read **72%** under this bug and **48%** correctly; TfLens computes it the pre-fix way today, so every cross-project figure it has published is wrong by about that margin. Making it a `record ReqKey(string Project, string ReqId)` rather than a convention means a rollup cannot be keyed on the id alone by *forgetting* the project — the omission does not compile. That is the ADR-007 technique on its fifth application, and this is the one place where a discipline-based fix would certainly have failed, because the bare key is the natural thing to write every time.
- **ADR-031 — `duration_s` and `attempt` are derived at read time, wrapped in their derived count, and never written back (2026-09-08).** Reason: both fields are absent from records that carry everything needed to compute them — **10 run records** with `started` and `ended` but no duration, **80 gate records** with no attempt, across the estate. Summing `duration_s` while skipping the first set counts those runs as **zero time while still counting their tokens**, so a phase's time and its tokens cover different sets of runs (the framework's own reset read 16h49m against a true 55h57m); dropping the second set from the first-pass rate removes them rather than failing them (twelve verdicts, all passing, reported **0%**). Deriving is not backfilling: TfLens writes to no stream at all (BRD §3), the computation runs every time a figure is built, and `rebuild` re-derives identical values from the raw archive — the same read-time discipline ADR-020 established for amendments. The value is returned as `DerivedFigure(T, int DerivedN)` because a derived total is legitimate **only while it is visibly derived**; an invisible derivation is indistinguishable from a measurement, which is the failure this whole product is built against.
- **ADR-032 — An unrecognised value in an open vocabulary is rendered and counted, never filtered away (2026-09-08).** Reason: `cmd`, gate `verdict` and `harness` are open at the producer and demonstrably grow — `cmd` gained five values on 2026-09-07 alone, and a whitelist built from the previous list would silently hide **27 records that already exist**. TfLens's own data carries a `rename-page` run and **37 off-list verdicts** (33 `PASS`, 2 `NOT-OBSERVABLE`, 2 `In Progress`). The asymmetry is the whole argument: showing an unknown value produces a **visible data-quality finding** that someone investigates, while dropping it produces a **wrong total that looks right** — and the same reasoning already governs `Overflow` for unknown *fields* (an unrecognised field goes to `Overflow`, never to `InvalidLines`). This ADR extends that settled rule from field names to field values. The one place a closed vocabulary is genuinely closed — `sort`'s four values — is handled the same way rather than differently: an out-of-vocabulary `sort` is stored, counted and surfaced as unrecognised, never rounded to the nearest legal value, because a miscategorised diagnosis is worse than an unknown one.
...
- **Three figures TfLens publishes today are wrong, and the fix is expected to move them** (2026-09-08, ADR-030 / ADR-031). Any cross-project rate keyed on the bare `req_id`, any time total summed from `duration_s`, and any first-pass rate over records lacking `attempt` are all wrong in the **same direction — the one that flatters the number**. The risk is not the correction but the reception: a first-pass rate that falls from roughly 72% to roughly 48% on the day the fix ships will read as a regression to anyone who does not know why, and any figure exported or quoted before that day was wrong by about that margin. The first extended parity run is **expected to fail**, and that failure is the evidence the fix was needed. Both facts belong in `DECISIONS.md` beside the parity record, not only in this document.
- **`sort` and `what` will be sparse for months, and `/misses` must be honest at that sparsity** (2026-09-08). The fields began on 2026-09-07; **286 of 359 misses across the estate carry neither**, and on TfLens's own repository *every* miss record predates them. This is the `observed_n = 1 of 13` situation again and it fails the same way: a page that looks broken while coverage is thin gets distrusted early and never recovers, and the natural "fix" — treating a record that predates the field as an *unsorted* one — is the pooling ADR-026 and BRD-172 both forbid. Filling the field in on older records is the owner's editorial call and something TfLens could not do in any case, since it writes to no stream.
...
- `docs/TfLens-Metrics-Update-Prompt.md` (2026-09-07) — the source of the 2026-09-08 amendment (BRD-170..BRD-181, ADR-029..ADR-032). Its §6 was **corrected on 2026-09-08**: the bullet that read *"No rate card, ever"* turned a producer rule into a consumer ban and contradicted both `.tfcore/telemetry/SCHEMA.md` — which the prompt itself names as the authority, and which bans presenting an estimate **as** a measurement, not a read-time estimate as such — and the framework's own design record. **Nothing in TfLens's repricing path changes:** BRD-62, BRD-123, BRD-128 and BRD-160 stand as written, the repricing panel and the price-list editor stay, ADR-009 is unaffected, and no requirement is struck. A future reader finding the uncorrected sentence in an older copy of the prompt should treat the schema as authoritative, as the prompt's own §7 instructs.

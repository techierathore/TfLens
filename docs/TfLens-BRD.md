...
**Snapshot as of 2026-09-02 (handoff)** — **169 of 179 requirements `Verified`**, 4 `Needs re-verify`, 4 `Implemented`, 1 `In Progress`, 1 `N/A`; **no `Blocked` rows**. Release build PASS, .NET **821/821**, Playwright **95/101**, render gate **489 controls / 22 screen-states with 0 non-RENDERS**, visual gate clean at 1280 and 390, `mockup-parity` **17 findings, none of them real**. The three UI defects the 2026-09-02 verify run found are closed (`REQ-UI-048`, `REQ-UI-038`, `REQ-UI-018`). The BRD §13 parity procedure was executed rather than assumed and found **three real engine defects** behind what had been recorded as a routine re-run — a stale store feeding `/effort`, an upward bias in the headline measured-cost figure, and a replay-path counting bug — all fixed; what remains is **one corrupt record in another repository's telemetry**, which is not TfLens's to repair (`REQ-FN-063`, `REQ-UI-033`, `TF-015`). Still owner-gated on a Playbook repository that exports: `REQ-UI-050`/`051`, `REQ-FN-067`/`070`. `REQ-FN-012` stays `N/A` (SSO deferred by BRD-94 / ADR-012). Live, per-requirement status: see `PROJECT-STATUS.md` and the **Requirements Status** table in `docs/TfLens-Checklist.md`.

| Feature (F-code) | Phase | Status | % | Notes |
|------------------|-------|--------|---|-------|
| F-SHELL: App shell and navigation | 1 | Done | 100 | Sidebar, eight-item nav, breadcrumb, theme toggle and header Framework switch all `Verified`. **`REQ-UI-010` is back to `Verified`** — the badge/track drift the 2026-08-29 parity gate found was fixed and has stayed clean through four subsequent runs. |
...
| F-3Q: Gate outcomes page *(renamed 2026-09-01; ID retained)* | 2 | **Done** | 100 | Per `project_type`, live vs backfilled columns, taint list. **`REQ-UI-018` closed 2026-09-02** — the trend line now strokes `--chart-1` on the polyline, matching the mockups' own cascade position; the app's last `color` finding is gone. The catch distribution also gained the two gates added upstream on 2026-08-31, `assets` and `mockup-parity`, each with its late-gate caveat badge (`REQ-FN-052`, `REQ-UI-020`). |
| F-HARN: Harness comparison page | 2 | Done | 100 | Three harness columns, tokens per verified REQ, the OpenCode-only dollars card and the `harness: null` footnote. **`REQ-UI-023` and `REQ-UI-025` are back to `Verified`** — the 2026-08-30 owner UAT evidence was addressed and both have held since. | Total tokens` table, an **11th `Measured dollars` row** against the mockup's ten (an owner call — the row satisfies `REQ-UI-025`'s own acceptance, so acceptance and approved design conflict), and a two-line card title. All four are **composition** drift, which `mockup-parity` compares elements rather than composition and cannot see (TF-011). Prior: claude-code / opencode / codex; OpenCode-only dollars, no cross-harness total. **Corrected 2026-08-27** — the measured figure summed `cost_usd` over *run* records, which never carry it (SCHEMA.md §4 puts it on sessions), so it was structurally null; the earlier `$0.84` came from test pollution in the shared DB. Now $0.04 over 2 OpenCode session records |
| F-ROUTE: Routing and economics page | 2 | Done | 100 | Drift, tokens by model, repricing from `prices.json`, poolables |
| F-MISS: Misses and rework economics | 3 | **Done** | 100 | Fifth stream (`misses.jsonl`, three record kinds), three tables, `MissMetrics` + the two provenance guards, the `/misses` page — all built and verified 2026-08-28. **`REQ-UI-038` closed 2026-09-02** — the `miss-type` tab strip now scrolls inside its own box (`::deep [role="tablist"]`), the rule `GateOutcomes.razor.css` already carried for the same control. `mockup-parity` reports nothing on this screen but the `app-sidebar` false positive. |
| F-EFFORT: Phase effort and efficiency | 3 | Partial | 90 | **Built 2026-09-01** (BRD-145..BRD-169), 27 REQs in one pass, engine diffed against the reference script at **401 figures, 0 diffs**. **`REQ-UI-048` closed 2026-09-02** — mono identifiers in the panel tables no longer break mid-token and the fan-out `median · max` pair stays whole at 390; the rule the mockup states at `effort.html:178` had simply never been carried into the build. **`REQ-UI-050`** is held `Implemented` on purpose: its acceptance is real Playbook figures and none exist yet — owner-gated on a repository that exports. |
| F-EXPORT: Weekly snapshot export | 2 | Partial | 90 | Button + `export` verb → markdown + JSON, both written for real; `REQ-UI-032` `Verified`. **`REQ-UI-033` held 2026-09-02** — the quotable banner reads NOT QUOTABLE. The banner is behaving correctly; what is behind it is a genuinely failing parity run (see F-PARITY), not a stale stamp. |
| F-PARITY: Parity check against tf-metrics.sh | 2 | Partial | 90 | `tflens.json` layout and `tools/parity-compare.py` verified both ways. **BRD §13 was RE-RUN on 2026-09-02 and does not pass — for reasons nobody had recorded.** Executed against a dataset fetched from the GitHub API at the pinned SHAs, with all 23 stream files verified hash-identical to TfLens's own archives. **62 findings → 4.** Three real TfLens defects, all now fixed: a **stale store** (rows ingested before the F-EFFORT columns existed were never updated on re-sync — 43 findings, cleared by `rebuild`), **`cost_sole_n` 10 vs 7** (the headline measured-cost figure, biased upward — `REQ-FN-079`), and **`session_duplicates_collapsed` 81 vs 7** on the replay path. Plus six MISSING `late_gate_coverage` keys (`REQ-FN-052`). The remaining **4 findings are one corrupt record in TechieBlog's telemetry** — a run whose `ended` precedes its `started` — which is that repository's to repair, not TfLens's; the producer-side gap is `TF-015`. **No figure is quotable until it is** (`REQ-FN-063`, `MISS-TfLens-20260902-07`). The stamp is working as BRD-71 specifies. |

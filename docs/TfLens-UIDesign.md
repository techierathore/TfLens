...
**Phase 1 of 3.** This file holds the six foundation screens plus the shared design system, the click-through flow and the library-gap log. Phase 2's five report screens are in [TfLens-P2-UIDesign.md](./TfLens-P2-UIDesign.md); phase 3's four are in [TfLens-P3-UIDesign.md](./TfLens-P3-UIDesign.md).
...
- **Shell revision 2026-09-11 (owner decision 4).** Below 768px the sidebar is off the page: the header's menu button (`sidebar-trigger`) slides it out from the left, 18rem wide with its labels, over a dark backdrop, as `SidebarProvider` draws it on a phone. The icon strip is a 768px-and-up state only.
...
flowchart LR
  ...
  RP --> C
  RP -. "sidebar: Price providers" .-> PRC["prices.html"]

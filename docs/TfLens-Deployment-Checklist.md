...
| Four org-level + five repo-level GitHub secrets (one time, §2), including **`TrBlazeUiPackagesToken`** — the GitHub Packages access token (classic PAT, `read:packages`) the image build needs to download TrBlazeUI; steps in §3 | owner |
...
| `TrBlazeUiPackagesToken` | GitHub Actions repo secret — a classic PAT with the `read:packages` scope, the same token the other techierathore applications use | The `build` job's preflight fails with one line naming this secret; without it the image cannot restore TrBlazeUI from the private GitHub Packages feed. (Alternative: grant this repository Read access on each TrBlazeUI package page; the built-in `GITHUB_TOKEN` then suffices) |
...
- [ ] Packages token (the image build restores TrBlazeUI from the private feed `https://nuget.pkg.github.com/techierathore/index.json`, which refuses anonymous reads; skip this item if you reuse the token your other applications use): GitHub → profile picture → **Settings** → **Developer settings** → **Personal access tokens** → **Tokens (classic)** → **Generate new token (classic)**, note `TfLens packages read`, tick **only** `read:packages`, **Generate token** and copy it — GitHub shows the token once and its scope list shows only `read:packages`
- [ ] Test the token: `curl -s -o /dev/null -w '%{http_code}\n' -u "techierathore:<token>" https://nuget.pkg.github.com/techierathore/download/trblazeui.components/index.json` — prints `200` (a `401` means the token or its scope is wrong)
- [ ] Save it: GitHub → the **TfLens** repository → **Settings** → **Secrets and variables** → **Actions** → **New repository secret**, name exactly `TrBlazeUiPackagesToken`, value = the token, **Add secret** — the repo's Actions secrets page lists `TrBlazeUiPackagesToken`
- [ ] On the first `deploy` run, open the `build` job's *Preflight — TrBlazeUI feed authentication* step — it prints `TrBlazeUI feed reachable and authenticated (HTTP 200).`; without the secret it stops the deploy here, naming the secret
...
| `build` job fails at *Preflight — TrBlazeUI feed authentication* with `Cannot read the private TrBlazeUI GitHub Packages feed (HTTP 401)` or `403` | The repo secret `TrBlazeUiPackagesToken` is missing, misspelt, expired, or lacks `read:packages` | Create a new token and save it under that exact name (§3), then re-run `deploy` via `workflow_dispatch` |
| `build` job fails in the image build with `NU1301` / `401 (Unauthorized)` although the preflight passed | The Dockerfile's restore did not receive the token: the `secrets:` line in `deploy.yml` no longer passes it as `nuget_pat` | Restore `nuget_pat=…` under `secrets:` in the build step; the id must match the Dockerfile's `--mount=type=secret,id=nuget_pat` |
| Site loads but nothing responds; `https://tflens.techierathore.com/_framework/blazor.web.js` returns `404` | The image was published without Blazor's framework files (the second restore in the Dockerfile was removed) | Put back the second restore after `COPY src/ src/` in the Dockerfile, then re-run `deploy` |

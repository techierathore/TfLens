...
| REQ-UI-005 | Profile page (BRD-107, BRD-95, Phase 1) | Implemented | 75% | 2026-08-27 `Components/Pages/Auth/Profile.razor`; smoked: profile-values table 5/5 non-empty rows, Role badge reads Manager. Mobile overflow fixed 2026-08-27 (grid track minmax(0,1fr), item min-width:0, fixed table layout, identity row shrink). | [view](#d-req-ui-005) |
...
| REQ-UI-008 | Header user menu (BRD-4, BRD-106, Phase 1) | Implemented | 75% | 2026-08-27 `Components/Shared/UserMenu.razor` — Avatar+name+chevron trigger, email label, Profile/Manage repos/Sign out; no bare sign-out control elsewhere. | [view](#d-req-ui-008) |
...
| REQ-FN-007 | Sign out — AppManager logout + cookie clear (BRD-4, Phase 1) | Implemented | 75% | 2026-08-26 — `POST /auth/logout` (antiforgery + `RequireAuthorization`) calls `/AuthSvc/logout`, deletes the session row and clears the cookie. An AppManager failure is logged by code and the local session is destroyed regardless. **Contract fix:** `IAppManagerClient.LogoutAsync` gained an optional `aAccessToken` — verified live that the endpoint answers **401 and revokes nothing** without the bearer header, so the original signature could not satisfy BRD-4. | [view](#d-req-fn-007) |
...
| REQ-FN-040 | One multi-stage Docker image with `data/` + `logs/` volumes (BRD-77, Phase 1) | Implemented | 75% | 2026-08-27 `Dockerfile` — multi-stage, data/ + logs/ volumes. | [view](#d-req-fn-040) |
